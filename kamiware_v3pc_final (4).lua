...
-- Path: ReplicatedStorage.Items.Emotes.{EmoteName}.EmoteInfo.SpeedMult = 2
-- =====================================================
...
local function applySpeedFix()
    ...
    if not EmotesFolder then
        warn("[SpeedFix] Could not find ReplicatedStorage.Items.Emotes")
        return 0
    end

    for _, emote in ipairs(EmotesFolder:GetChildren()) do
        local ok, data = pcall(require, emote)
        if ok and type(data) == "table" then
            local info = data.EmoteInfo
            if type(info) == "table" and info.SpeedMult ~= nil then
                if SpeedFixOriginals[info] == nil then
                    SpeedFixOriginals[info] = info.SpeedMult
                end
                info.SpeedMult = 2
                patched = patched + 1
            end
