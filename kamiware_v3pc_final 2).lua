...
-- Mirrors the proven NonMovableEmoteHop pattern exactly,
-- but patches ALL emotes (any SpeedMult value) → sets to 2.
-- =====================================================
...
local function applySpeedFix()
    ...
    for _, module in pairs(EmotesFolder:GetDescendants()) do
        if module:IsA("ModuleScript") then
            ...
            if success and type(emoteData) == "table" and emoteData["EmoteInfo"] then
                local emoteInfo = emoteData["EmoteInfo"]
                if type(emoteInfo) == "table" and emoteInfo["SpeedMult"] ~= nil then
                    if SpeedFixOriginals[emoteInfo] == nil then
                        SpeedFixOriginals[emoteInfo] = emoteInfo["SpeedMult"]
                    end
                    emoteInfo["SpeedMult"] = 2
                    fixedCount = fixedCount + 1
                end
            end
        end
