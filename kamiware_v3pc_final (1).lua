...
-- Collect all AnimationIds from the Moving folder at load time
local movingAnimIds = {}
pcall(function()
    ...
    if movingFolder then
        for _, obj in ipairs(movingFolder:GetDescendants()) do
            if obj:IsA("Animation") and obj.AnimationId ~= "" then
                movingAnimIds[obj.AnimationId] = true
            end
        end
    end
end)
...
local function isMovingAnimPlaying()
    ...
    for _, track in ipairs(animator:GetPlayingAnimationTracks()) do
        if track.IsPlaying then
            ...
            if movingAnimIds[id] then return true end
        end
    end
    return false
end
...
local function setArmTransparency(t)
    local char = LocalPlayer.Character
    if not char then return end
...
function FOVBodyVisibility.Enable()
    ...
    FOVBodyVisibility.Connection = RunService.RenderStepped:Connect(function()
        ...
        setArmTransparency(isMovingAnimPlaying() and 1 or 0)
    end)
end
