...
-- Build a set of all AnimationId strings found under the Moving folder
local movingAnimIds = {}
pcall(function()
    local movingFolder = ReplicatedStorage
        :WaitForChild("Objects", 10)
        :WaitForChild("Game", 10)
        :WaitForChild("Character", 10)
        :WaitForChild("Animations", 10)
        :WaitForChild("Types", 10)
        :WaitForChild("Classic", 10)
        :WaitForChild("Default", 10)
        :WaitForChild("Animations", 10)
        :WaitForChild("Moving", 10)

    if movingFolder then
        for _, anim in ipairs(movingFolder:GetDescendants()) do
            if anim:IsA("Animation") and anim.AnimationId ~= "" then
                movingAnimIds[anim.AnimationId] = true
            end
        end
    end
end)

local function isMovingAnimPlaying()
    local char = LocalPlayer.Character
...
local function isInFirstPerson()
    ...
    -- First person: camera very close to head (distance < 1)
    local char = LocalPlayer.Character
...
function FOVBodyVisibility.Restore()
    ...
    for _, part in ipairs(myRig:GetChildren()) do
        if part:IsA("BasePart") and FOVBodyVisibility.ArmParts[part.Name] then
...
function FOVBodyVisibility.Enable()
    ...
    FOVBodyVisibility.Connection = RunService.RenderStepped:Connect(function()
        ...
        if not isInFirstPerson() then
            -- Restore arms when not in first person
            for _, part in ipairs(myRig:GetChildren()) do
                if part:IsA("BasePart") and FOVBodyVisibility.ArmParts[part.Name] then
                    part.LocalTransparencyModifier = 0
                end
        ...
        local hide = isMovingAnimPlaying()
        for _, part in ipairs(myRig:GetChildren()) do
            if part:IsA("BasePart") and FOVBodyVisibility.ArmParts[part.Name] then
