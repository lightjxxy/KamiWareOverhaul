...
-- ─── UI Scale ────────────────────────────────────────────────────────────────
-- UIScale lives directly inside MainFrame (already in scope here)
local _uiScaleObj = Instance.new("UIScale")
_uiScaleObj.Scale = 1
_uiScaleObj.Parent = MainFrame

local _uiBaseW = MainFrame.Size.X.Offset  -- 836
local _uiBaseH = MainFrame.Size.Y.Offset  -- 538
...
local function _getAutoScale()
    ...
    -- Fit UI to screen with a small margin; take the tighter axis
    local s = math.min((vw * 0.96) / _uiBaseW, (vh * 0.96) / _uiBaseH)
    return math.floor(math.clamp(s, 0.30, 1.00) * 100)
end

local function _applyUIScale(pct)
    local scale = pct / 100
    _uiScaleObj.Scale = scale
    -- Re-centre MainFrame to account for the new visual size
    local hw = (_uiBaseW * scale) / 2
    local hh = (_uiBaseH * scale) / 2
    MainFrame.Position = UDim2.new(0.5, -hw, 0.5, -hh)
end

-- Apply immediately on load (no delay needed; MainFrame is already created)
task.defer(function()
    _applyUIScale(_getAutoScale())
end)
