...
-- =====================================================
-- MOVEMENT WARE V3 - LINORIA MIGRATED VERSION
...
LocalPlayer.CharacterAdded:Connect(function(char)
    ...
    if Settings.AutoApplyDraconicEmoteSwapper then
        ...
        if swappedCount == 0 and next(DraconicSwapper.OriginalPairs) then
            task.wait(2)
            for targetEmote, replacementEmote in pairs(DraconicSwapper.OriginalPairs) do
                if draconicSwapEmote(targetEmote, replacementEmote) then
                    swappedCount = swappedCount + 1
                end
            end
        end
...
-- =====================================================
-- MOVEMENT WARE V3 - KAMIBLOX UI SETUP
-- =====================================================
...
local SpdTab = Window:CreateTab("Speed", "112728826405675")
...
MovLeft:CreateSlider({ Name = "Strafe Acceleration", Min = 1, Max = 1000, Default = 182,
    Callback = function(v) AirStrafeAcceleration = v; updateAirStrafeValues() end })
MovLeft:CreateSlider({ Name = "Jump Height", Min = 1, Max = 10, Default = 3,
    Callback = function(v) AirStrafeJumpHeight = v; updateAirStrafeValues() end })
...
MovLeft:CreateSlider({ Name = "Strafe Intensity", Min = 100, Max = 2000, Default = 500,
...
MovRight:CreateTextbox({ Name = "Y Lock Key (e.g. X)", Placeholder = "X",
    Callback = function(v)
...
local SpdLeft = SpdTab:CreateSection({ Name = "Speed Changer", Side = "Left" })


SpdLeft:CreateToggle({
    Name = "Overhaul Speed", Default = false,
    Callback = function(v)
        OverhaulSpeedBoosterEnabled = v
        if v then
            startOverhaulSpeedRespawnHook(); applyOverhaulSpeedBooster()
...
SpdLeft:CreateTextbox({ Name = "Overhaul Speed Value", Placeholder = "2000",
    Callback = function(v)
        local n = tonumber(v)
        if n then OverhaulSpeedValue = math.max(n, 0)
            if OverhaulSpeedBoosterEnabled then applyOverhaulSpeedBooster() end
        end
    end
})

local SpdRight = SpdTab:CreateSection({ Name = "Hitbox Creator", Side = "Right" })

SpdRight:CreateToggle({
...
VisRight:CreateButton({
    ...
    Callback = function()
        ...
        for i = 1, 6 do
            ...
            if cur ~= "" and sel ~= "" then
                local cOk, cAct = isValidEmote(cur)
                local sOk, sAct = isValidEmote(sel)
                if cOk and sOk and cAct ~= sAct then
                    if draconicSwapEmote(cAct, sAct) then swapped = swapped + 1
...
VisRight:CreateButton({ Name = "Restore All Emotes",
    Callback = function()
        if draconicRestoreAllEmotes(false) then notify("All emotes restored.", 3)
        else notify("No emotes to restore.", 3) end
    end
})
...
local WorldRight = WorldTab:CreateSection({ Name = "Cosmetics", Side = "Right" })
...
WorldRight:CreateButton({ Name = "Apply Cosmetic Style Now",
    Callback = function()
...
local cosSwapOwned = ""
local cosSwapWanted = ""
WorldRight:CreateTextbox({ Name = "Owned Cosmetic", Placeholder = "FrostFlame",
    Callback = function(v) cosSwapOwned = v end })
WorldRight:CreateTextbox({ Name = "Wanted Cosmetic", Placeholder = "Broom",
    Callback = function(v) cosSwapWanted = v end })
WorldRight:CreateButton({ Name = "Apply Cosmetic Swap",
    Callback = function()
        pcall(function()
            local fld = ReplicatedStorage:FindFirstChild("Items") and ReplicatedStorage.Items:FindFirstChild("Cosmetics")
            if fld and fld:FindFirstChild(cosSwapOwned) and fld:FindFirstChild(cosSwapWanted) then
                for _, c in pairs(fld[cosSwapOwned]:GetChildren()) do c:Destroy() end
                for _, c in pairs(fld[cosSwapWanted]:GetChildren()) do c:Clone().Parent = fld[cosSwapOwned] end
