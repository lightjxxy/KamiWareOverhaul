...
-- Mobile detection: true on a touch device without a physical keyboard
...
function Library:CreateWindow(options)
    ...
    -- On PC the key hides/shows the whole ScreenGui.
    -- On mobile we toggle MainFrame visibility (closure ref filled after creation).
    local _mainFrameRef = nil
    UserInputService.InputBegan:Connect(function(input, gameProcessed)
        if not gameProcessed and input.KeyCode == toggleKeyState.key then
            if _isMobile then
                if _mainFrameRef then _mainFrameRef.Visible = not _mainFrameRef.Visible end
            else
    ...
    local function OpenColorPicker(title, defaultColor, defaultAlpha, callback, sourceElement)
        OpenModal(title, function(container)
            ...
            local function _isPickInput(i) return i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch end
            SVMap.InputBegan:Connect(function(input) if _isPickInput(input) then draggingSV = true; updateSV(input) end end)
            HueBar.InputBegan:Connect(function(input) if _isPickInput(input) then draggingHue = true; updateHue(input) end end)
            AlphaBar.InputBegan:Connect(function(input) if _isPickInput(input) then draggingAlpha = true; updateAlpha(input) end end)
            
            local c1 = UserInputService.InputEnded:Connect(function(input) 
                if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then draggingSV, draggingHue, draggingAlpha = false, false, false end 
            end)
    ...
    _mainFrameRef = MainFrame

    -- Mobile: floating button to show/hide the main window ───────────────────
    if _isMobile then
        local MobileToggleBtn = Utility:Create("TextButton", {
            Name = "MobileToggleBtn", Parent = ScreenGui,
            BackgroundColor3 = Theme.TabButton, BackgroundTransparency = 0.1,
            BorderSizePixel = 0,
            Position = UDim2.new(0, 10, 0.5, -22),
            Size = UDim2.new(0, 44, 0, 44),
            Text = "KW", Font = Theme.Font,
            TextColor3 = Theme.TextMain, TextSize = 11,
            ZIndex = 200, Active = true
        })
        Utility:Create("UICorner", { Parent = MobileToggleBtn, CornerRadius = UDim.new(0, 8) })
        Utility:MakeDraggable(MobileToggleBtn, MobileToggleBtn)
    ...
    local TabButtonContainer = Utility:Create("ScrollingFrame", {
        ...
        ScrollBarImageColor3 = Theme.TextCategory,
    ...
    function Window:CreateTab(name, iconId)
        ...
        function TabObj:CreateSection(options)
            ...
            function Elements:CreateToggle(opts)
                ...
                -- Shared helper: opens the keybind modal (used by both click methods)
                ...
                -- Left-click: colorpicker if available; otherwise keybind modal.
                -- On mobile right-click does not exist, so this gives access to both.
                ...
                -- Right-click (PC only): always opens the keybind modal
                GearBtn.MouseButton2Click:Connect(openKeybindModal)
...
-- =====================================================
-- CLONE SPAWNER - State & Logic
-- =====================================================

local PhysicsService = game:GetService("PhysicsService")

local CLONE_GROUP = "R6ClonesGroup"
pcall(function()
    PhysicsService:RegisterCollisionGroup(CLONE_GROUP)
    PhysicsService:CollisionGroupSetCollidable(CLONE_GROUP, "Default", false)
end)

local CloneSpawnerEnabled = false
local CloneSpawnerMobileBtn = nil
local _spawnedClones = {} -- track clones for delete-all

local function spawnR6Model()
    if not CloneSpawnerEnabled then return end

    local playerCache = Workspace:FindFirstChild("PlayerCache")
    if not playerCache then warn("CloneSpawner: PlayerCache not found!") return end

    local targetFolder = playerCache:FindFirstChild("nymnmz")
    if not targetFolder then warn("CloneSpawner: nymnmz folder not found!") return end

    local r6Template = targetFolder:FindFirstChild("R6")
    if not r6Template then warn("CloneSpawner: R6 model not found!") return end

    local character = LocalPlayer.Character
    if not character or not character:FindFirstChild("HumanoidRootPart") then
        warn("CloneSpawner: Character or HumanoidRootPart not found!")
        return
    end

    local clonedR6 = r6Template:Clone()
    local targetCFrame = character.HumanoidRootPart.CFrame + Vector3.new(0, 1.5, 0)

    if clonedR6.PrimaryPart then
        clonedR6:SetPrimaryPartCFrame(targetCFrame)
    else
        clonedR6:PivotTo(targetCFrame)
    end

    local humanoid = clonedR6:FindFirstChildOfClass("Humanoid")
    if humanoid then
    ...
    for _, part in ipairs(clonedR6:GetDescendants()) do
        if part:IsA("BasePart") then
            part.CanCollide = true
            part.Anchored = false
            pcall(function()
                PhysicsService:SetPartCollisionGroup(part, CLONE_GROUP)
            end)
        end
    end

    clonedR6.Parent = Workspace
    table.insert(_spawnedClones, clonedR6)

    -- Clean up destroyed clones from tracking table
    clonedR6.Destroying:Connect(function()
        for i, c in ipairs(_spawnedClones) do
            if c == clonedR6 then table.remove(_spawnedClones, i) break end
        end
    end)
end

local function deleteAllClones()
    for _, c in ipairs(_spawnedClones) do
        pcall(function() c:Destroy() end)
    end
    _spawnedClones = {}
end

-- Q keypress to spawn (only when enabled)
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if gameProcessed then return end
    if CloneSpawnerEnabled and input.KeyCode == Enum.KeyCode.Q then
        spawnR6Model()
    end
end)
...
-- =====================================================
-- SATURATION CONTROLLER (integrated into KamiWare)
-- =====================================================

local saturationEnabled = false
local saturationValue = 0 -- UI Value (-100 to 100)
...
-- =====================================================
-- AUTO BENCH VARIABLES
-- =====================================================

local AutoBenchEnabled = false
local benchBounceHeights = {100, 70, 80, 90}
local benchDebounceMap = {}
local benchConnections = {}

local function applyToBenchModel(model)
    if model:IsA("Model") or model:IsA("Folder") then
        for _, descendant in ipairs(model:GetDescendants()) do
            if descendant:IsA("BasePart") then
                local conn = descendant.Touched:Connect(function(hit)
                    if not AutoBenchEnabled then return end
                    local character = hit.Parent
                    local humanoid = character:FindFirstChildOfClass("Humanoid")
                    local rootPart = character:FindFirstChild("HumanoidRootPart")
                    if humanoid and rootPart then
                        if benchDebounceMap[character] then return end
                        benchDebounceMap[character] = true
                        humanoid:ChangeState(Enum.HumanoidStateType.Freefall)
                        local selectedHeight = benchBounceHeights[math.random(1, #benchBounceHeights)]
                        ...
                        task.delay(0.5, function()
                            benchDebounceMap[character] = nil
                        end)
                    end
                end)
                table.insert(benchConnections, conn)
            end
        end
        local dConn = model.DescendantAdded:Connect(function(descendant)
            if not AutoBenchEnabled then return end
            if descendant:IsA("BasePart") then
                local conn = descendant.Touched:Connect(function(hit)
                    if not AutoBenchEnabled then return end
        ...
        table.insert(benchConnections, dConn)
    end
end

local function enableAutoBench()
    for _, conn in ipairs(benchConnections) do conn:Disconnect() end
    benchConnections = {}
    benchDebounceMap = {}
    ...
    -- Apply to all existing Bench models anywhere under Parts
    ...
    -- Watch for any NEW Bench models added anywhere under Parts (covers ImmovableProps, Map subfolder, etc.)
    local conn = parts.DescendantAdded:Connect(function(descendant)
        if not AutoBenchEnabled then return end
        if descendant:IsA("Model") and descendant.Name == "Bench" then
            applyToBenchModel(descendant)
        end
...
function rebuildDeepWineSequence()
...
function replaceEmoteContents(targetName, sourceName)
...
function revertEmoteContents(targetName)
    local target = findItemByName(targetName)
    if not target then
...
function saveOriginalFogSettings()
...
function removePerfectFogObjects()
...
local SUNSET_SHADER_PREFIX = "KWSunsetShader"

function saveSunsetShaderLighting()
...
function removeSunsetShaderObjects()
    for _, obj in ipairs(Lighting:GetChildren()) do
    ...
    local overlay = playerGui and playerGui:FindFirstChild("KWSunsetShaderOverlay")
    if overlay then
        overlay:Destroy()
    end
end

function restoreSunsetShaderLighting()
...
function applySunsetShader()
    ...
    bloomEffect.Name = "KWSunsetShaderBloom"
    ...
    blurEffect.Name = "KWSunsetShaderBlur"
    blurEffect.Size = 5
    blurEffect.Parent = Lighting

    local colorCorrectionEffect = Instance.new("ColorCorrectionEffect")
    colorCorrectionEffect.Name = "KWSunsetShaderColorCorrection"
    ...
    atmosphere.Name = "KWSunsetShaderAtmosphere"
    ...
    screenGui.Name = "KWSunsetShaderOverlay"
...
function updateVirtualStrafe(dt)
    ...
    -- On mobile WASD keys are never active; derive directions from MoveDirection.
    local isW, isA, isD, isS
    if _isMobile then
        local _hum = char:FindFirstChildOfClass("Humanoid")
        local _md  = _hum and _hum.MoveDirection or Vector3.zero
        local _camF = Vector3.new(camera.CFrame.LookVector.X, 0, camera.CFrame.LookVector.Z)
        local _camR = Vector3.new(camera.CFrame.RightVector.X, 0, camera.CFrame.RightVector.Z)
        if _camF.Magnitude > 0 then _camF = _camF.Unit end
        if _camR.Magnitude > 0 then _camR = _camR.Unit end
        local _fwd   = _md:Dot(_camF)
        local _right = _md:Dot(_camR)
        isW = _fwd   >  0.3
        isS = _fwd   < -0.3
        isA = _right < -0.3
        isD = _right >  0.3
    else
        isW = UserInputService:IsKeyDown(Enum.KeyCode.W)
        isA = UserInputService:IsKeyDown(Enum.KeyCode.A)
        isD = UserInputService:IsKeyDown(Enum.KeyCode.D)
        isS = UserInputService:IsKeyDown(Enum.KeyCode.S)
    end
...
function refreshEasyBounceCharacter(char)
...
function simulateEasyBounceStep(dt)
...
function smoothEasyBounce(a, b, t)
    return a + (b - a) * t
end
...
function getAirStrafeBaseStats()
...
function applyAirStrafeModifications()
    pcall(function()
...
function restoreAirStrafeModifications()
    pcall(function()
        local stats = getAirStrafeBaseStats()
...
function updateAirStrafeValues()
...
function detectaObjeto(direcao, distancia)
    local character = LocalPlayer.Character
...
function clearCrouchConnections()
...
local KORBLOX_COLOR_ATTR = "KWOriginalKorbloxLegColor"
local KORBLOX_MATERIAL_ATTR = "KWOriginalKorbloxLegMaterial"
local KORBLOX_TRANSPARENCY_ATTR = "KWOriginalKorbloxLegTransparency"
local HEADLESS_TRANSPARENCY_ATTR = "KWOriginalHeadlessTransparency"
local avatarCosmeticRetryToken = 0

function applyDeepWineLogic(target, customColor)
    if not target then
        return
    end
...
function getKorbloxRightLeg(char)
    if not char then
        return nil
    end
...
function saveKorbloxLegOriginals(char, rLeg)
...
function saveOriginalTransparency(obj)
...
function applyAvatarCosmeticsWithRetries(char)
...
function applyNonMovableEmoteHop()
...
function restoreNonMovableEmoteHop()
...
local lastFP = nil
function updateVFXFirstPerson(char)
    if not char then
        return
    end
    local head = char:FindFirstChild("Head")
...
function inputMatchesYLockKey(input)
...
function hookCameraFOVSignal(cam)
...
LocalPlayer.CharacterAdded:Connect(function(char)
    ...
    -- Reapply saturation after respawn
...
function loadMapByAssetId(mapAssetID)
...
-- =====================================================
-- MOBILE FLY CONTROLS (on-screen Up / Down buttons)
-- =====================================================

local _mobileFlyGui = nil

function createMobileFlyControls()
    if _mobileFlyGui then _mobileFlyGui:Destroy(); _mobileFlyGui = nil end
    if not _isMobile then return end

    _mobileFlyGui = Instance.new("ScreenGui")
    _mobileFlyGui.Name = "KWMobileFlyControls"
    _mobileFlyGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    _mobileFlyGui.ResetOnSpawn = false
    local _ok = pcall(function() _mobileFlyGui.Parent = game:GetService("CoreGui") end)
    if not _ok then _mobileFlyGui.Parent = LocalPlayer:WaitForChild("PlayerGui") end

    local function makeBtn(lbl, posY)
        local b = Instance.new("TextButton")
        b.BackgroundColor3 = Color3.fromRGB(44, 44, 44)
        b.BackgroundTransparency = 0.15
        b.BorderSizePixel = 0
        b.Position = UDim2.new(1, -100, 1, posY)
    ...
    local FlyUpBtn   = makeBtn("UP",   -148)
    local FlyDownBtn = makeBtn("DOWN", -84)

    FlyUpBtn.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Touch then subindoQ = true end
    end)
    FlyUpBtn.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Touch then subindoQ = false end
    end)
    FlyDownBtn.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Touch then
            descendoCTRL = true; vAtualDescida = 0
        end
    end)
    FlyDownBtn.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Touch then
            descendoCTRL = false; vAtualDescida = 0; emEstadoDeBounce = false
        end
    end)
end

function destroyMobileFlyControls()
    if _mobileFlyGui then _mobileFlyGui:Destroy(); _mobileFlyGui = nil end
    subindoQ = false; descendoCTRL = false
    vAtualDescida = 0; emEstadoDeBounce = false
end

-- =====================================================
-- LAGSWITCH MOBILE BUTTON HELPER
-- =====================================================

--[[
    Creates a draggable, lockable floating button for mobile lagswitch use.
    - Tap the button to trigger the action (when NOT locked).
    - Long-press (~0.4 s) to enter drag mode; drag to reposition.
    - Double-tap the lock icon (🔒/🔓) in the corner to toggle lock.
    When locked the button stays in place and tapping it still fires the action.
    Returns the ScreenGui so the caller can destroy it on disable.
]]
local function makeLagswitchMobileBtn(guiName, label, defaultPos, onActivate)
    local gui = Instance.new("ScreenGui")
    gui.Name = guiName
    gui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    gui.ResetOnSpawn = false
    local _ok = pcall(function() gui.Parent = game:GetService("CoreGui") end)
    if not _ok then gui.Parent = LocalPlayer:WaitForChild("PlayerGui") end
    ...
    -- Small lock indicator in the top-right corner of the button
    ...
    local function setLocked(state)
        ...
        btn.BackgroundColor3 = isLocked
            and Color3.fromRGB(60, 30, 30)
            or  Color3.fromRGB(44, 44, 44)
    end
    ...
    -- Drag logic via InputBegan/InputChanged/InputEnded on the button
    btn.InputBegan:Connect(function(input)
        ...
        -- Start long-press timer to begin drag (only if not locked)
        if not isLocked then
            longPressThread = task.delay(LONG_PRESS_TIME, function()
                isDragging = true
                dragStart = input.Position
    ...
    btn.InputEnded:Connect(function(input)
        ...
        if isDragging then
            -- Finished drag: snap to final position, don't fire action
            isDragging = false
            btn.BackgroundTransparency = 0.15
        else
            -- Short tap: fire the action
            onActivate()
        end
    end)
...
local LagswitchFreezeMs = 300
local LagswitchKey = Enum.KeyCode.E
local LagswitchIsBinding = false
local LagswitchMobileBtn = nil

local function triggerLagswitch()
    if not LagswitchEnabled then return end
    local seconds = LagswitchFreezeMs / 1000
...
UserInputService.InputBegan:Connect(function(input, gp)
    ...
    if LagswitchIsBinding then return end
    if LagswitchEnabled and input.KeyCode == LagswitchKey then
        triggerLagswitch()
    end
end)
...
local FFlagLagswitchEnabled = false
local FFlagLagswitchFreezeMs = 300
local FFlagLagswitchKey = Enum.KeyCode.E
local FFlagLagswitchIsBinding = false
local FFlagLagswitchMobileBtn = nil

local fflagLagswitchApplied = false
local function applyFFlagLagswitchFlags()
    if fflagLagswitchApplied then return end
    fflagLagswitchApplied = true
    pcall(function()
        if setfflag then
            setfflag("MaxMissedWorldStepsRemembered", "2147483467")
        end
    end)
end

local function triggerFFlagLagswitch()
    if not FFlagLagswitchEnabled then return end
    local seconds = FFlagLagswitchFreezeMs / 1000
    local start = os.clock()
    while os.clock() - start < seconds do end
end
...
UserInputService.InputBegan:Connect(function(input, gp)
    if gp then return end
    if FFlagLagswitchIsBinding then return end
    if FFlagLagswitchEnabled and input.KeyCode == FFlagLagswitchKey then
        triggerFFlagLagswitch()
    end
end)
...
function applyOverhaulSpeedBooster()
...
function restoreOverhaulSpeedBooster()
    if overhaulSpeedOriginal == nil then
        return
    end
...
-- Jump Cap
local JumpCapEnabled = false
local JumpCapValue = 1
local jumpCapOriginal = nil

function getBaseStats()
    return require(game:GetService("ReplicatedStorage").Objects.Game.Character.Client.Movement.MoveStats.BaseStats)
end

function applyJumpCap()
    local BaseStats = getBaseStats()
    if jumpCapOriginal == nil then
        jumpCapOriginal = BaseStats.JumpCap
    end
    BaseStats.JumpCap = JumpCapValue
end

function restoreJumpCap()
    if jumpCapOriginal == nil then return end
    local BaseStats = getBaseStats()
    BaseStats.JumpCap = jumpCapOriginal
    jumpCapOriginal = nil
end

MovLeft:CreateToggle({
    Name = "Jump Cap", Default = false,
    Callback = function(v)
        JumpCapEnabled = v
        if v then
            applyJumpCap()
            notify("Jump Cap enabled. Jumps: "..tostring(JumpCapValue), 2)
        else
            restoreJumpCap(); notify("Jump Cap disabled.", 2)
        end
    end
})
MovLeft:CreateTextbox({ Name = "Jump Cap Value", Placeholder = "1",
    Callback = function(v)
        local n = tonumber(v)
        if n and n >= 1 then
            JumpCapValue = math.floor(n)
            if JumpCapEnabled then
                applyJumpCap()
                notify("Jump Cap changed to: "..tostring(JumpCapValue), 2)
            end
        else
            notify("Enter a number of 1 or higher.", 3)
        end
    end
})
...
MovLeft:CreateToggle({
    ...
    Callback = function(v)
        if _isMobile then
            -- On mobile the Q/Ctrl keys don't exist; use the on-screen UP/DOWN buttons
            Settings.MobileFlyEnabled = v
            if v then createMobileFlyControls() else destroyMobileFlyControls() end
        else
...
MovRight:CreateToggle({
    ...
    Callback = function(v)
        ...
        if _isMobile then
            if v then
                if not LagswitchMobileBtn then
                    LagswitchMobileBtn = makeLagswitchMobileBtn("KWLagswitchBtn", "LAG", UDim2.new(0, 10, 1, -170), triggerLagswitch)
                end
            else
                if LagswitchMobileBtn then LagswitchMobileBtn:Destroy(); LagswitchMobileBtn = nil end
            end
        end
        notify(v and "Lagswitch enabled. Press " .. LagswitchKey.Name .. " to freeze." or "Lagswitch disabled.", 2)
    end
})
MovRight:CreateTextbox({
    Name = "Lagswitch Freeze (ms)",
    Placeholder = "300",
    Callback = function(v)
        local n = tonumber(v)
        if n and n > 0 then
            LagswitchFreezeMs = n
            notify("Lagswitch freeze set to " .. n .. "ms", 2)
        else
            notify("Enter a valid number.", 3)
        end
    end
})
MovRight:CreateKeybind({
    Name = "Lagswitch Key",
    Default = "E",
    Callback = function(v)
        local key = Enum.KeyCode[v]
...
-- FFLAG LAGSWITCH
MovRight:CreateToggle({
    Name = "FFlag Lagswitch",
    Default = false,
    Callback = function(v)
        FFlagLagswitchEnabled = v
        if v then
            applyFFlagLagswitchFlags()
        end
        -- Mobile button
        if _isMobile then
            if v then
                if not FFlagLagswitchMobileBtn then
                    FFlagLagswitchMobileBtn = makeLagswitchMobileBtn("KWFFlagLagswitchBtn", "FLAG", UDim2.new(0, 10, 1, -220), triggerFFlagLagswitch)
                end
            else
                if FFlagLagswitchMobileBtn then FFlagLagswitchMobileBtn:Destroy(); FFlagLagswitchMobileBtn = nil end
            end
        end
        notify(v and "FFlag Lagswitch enabled. Press " .. FFlagLagswitchKey.Name .. " to freeze." or "FFlag Lagswitch disabled.", 2)
    end
})
MovRight:CreateTextbox({
    Name = "FFlag Lagswitch Freeze (ms)",
    Placeholder = "300",
    Callback = function(v)
        local n = tonumber(v)
        if n and n > 0 then
            FFlagLagswitchFreezeMs = n
            notify("FFlag Lagswitch freeze set to " .. n .. "ms", 2)
        else
            notify("Enter a valid number.", 3)
        end
    end
...
-- AUTO BENCH SECTION (Movement Tab - Right Side)
...
MovBench:CreateTextbox({
    ...
    Callback = function(v)
        local n = tonumber(v)
        if n then
            benchBounceHeights[1] = n
            notify("Bounce Height 1 set to: " .. tostring(n), 2)
        else
            notify("Enter a valid number.", 3)
        end
    end
...
MovBench:CreateTextbox({
    ...
    Callback = function(v)
        local n = tonumber(v)
        if n then
            benchBounceHeights[2] = n
            notify("Bounce Height 2 set to: " .. tostring(n), 2)
        else
            notify("Enter a valid number.", 3)
        end
    end
...
MovBench:CreateTextbox({
    ...
    Callback = function(v)
        local n = tonumber(v)
        if n then
            benchBounceHeights[3] = n
            notify("Bounce Height 3 set to: " .. tostring(n), 2)
        else
            notify("Enter a valid number.", 3)
        end
    end
...
MovBench:CreateTextbox({
    ...
    Callback = function(v)
        local n = tonumber(v)
        if n then
            benchBounceHeights[4] = n
            notify("Bounce Height 4 set to: " .. tostring(n), 2)
        else
            notify("Enter a valid number.", 3)
        end
    end
...
-- FOV Body Visibility (First Person Body Fix)
local FOVBodyVisibilityEnabled = false
local fovBodyConn = nil
...
local function fovIsFirstPerson(rig)
    local head = rig:FindFirstChild("Head") or (LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Head"))
    if head then
        return (camera.CFrame.Position - head.Position).Magnitude < 2
    end
    return (camera.CFrame.Position - camera.Focus.Position).Magnitude < 1
end

local function fovIsMovingOrJumping(rig)
    local humanoid = rig:FindFirstChildOfClass("Humanoid") or (LocalPlayer.Character and LocalPlayer.Character:FindFirstChildOfClass("Humanoid"))
    ...
    if animator then
        for _, track in ipairs(animator:GetPlayingAnimationTracks()) do
            ...
            if track.IsPlaying and (animName:find("run") or animName:find("walk") or animName:find("jump") or animName:find("sprint")) then
                return true
            end
        end
    end
...
function resetFOVBodyVisibility()
    local rigsFolder = workspace:FindFirstChild("Rigs")
    if not rigsFolder then return end
    local myRig = rigsFolder:FindFirstChild(LocalPlayer.Name) or rigsFolder:FindFirstChild("hfurhutu")
...
function stepFOVBodyVisibility()
    local rigsFolder = workspace:FindFirstChild("Rigs")
    if not rigsFolder then return end
    ...
    local inFirstPerson = fovIsFirstPerson(myRig)

    -- Third person: reset all transparency to normal
    ...
    -- First person: apply custom FOV & movement-based visibility
    local isHighFOV = camera.FieldOfView > 90
    local actionActive = fovIsMovingOrJumping(myRig)

    for _, part in ipairs(myRig:GetChildren()) do
        if part:IsA("BasePart") then
            if isHighFOV then
                if fovArmParts[part.Name] then
                    -- Hide arms when moving/jumping in high FOV, show when still
                    if actionActive then
                        part.LocalTransparencyModifier = 1
                    else
                        part.LocalTransparencyModifier = part.Transparency
                    end
                elseif fovLowerBodyParts[part.Name] then
                    -- Keep lower body visible in high FOV first person
                    part.LocalTransparencyModifier = part.Transparency
                end
            else
                -- Standard FOV in first person: hide body
                if fovArmParts[part.Name] or fovLowerBodyParts[part.Name] then
                    part.LocalTransparencyModifier = 1
                end
            end
        end
...
VisLeft:CreateToggle({
    Name = "FOV Body Visibility", Default = false,
    Callback = function(v)
        FOVBodyVisibilityEnabled = v
        if v then
            if not fovBodyConn then
                fovBodyConn = game:GetService("RunService").RenderStepped:Connect(function()
                    if not FOVBodyVisibilityEnabled then return end
                    stepFOVBodyVisibility()
                end)
            end
            notify("FOV Body Visibility enabled.", 2)
        else
            if fovBodyConn then fovBodyConn:Disconnect(); fovBodyConn = nil end
            resetFOVBodyVisibility()
...
function replaceCosmeticContents(targetName, sourceName)
...
function revertCosmeticContents(targetName)
    local target = ReplicatedStorage:FindFirstChild(targetName, true)
    if not target then
...
-- =====================================================
-- SATURATION SECTION (Visuals Tab)
-- =====================================================
...
VisSaturation:CreateTextbox({
    Name = "Saturation Value (-100 to 100)",
    Placeholder = "0",
    Callback = function(v)
        local n = tonumber(v)
        if n then
            ...
            notify("Enter a number between -100 and 100.", 3)
        end
    end
})
...
-- ─── Clone Spawner ───────────────────────────────────────────────────────────
local CloneSection = WorldTab:CreateSection({ Name = "Clone Spawner", Side = "Left" })

CloneSection:CreateToggle({
    Name = "Enable Clone Spawner",
    Default = false,
    Callback = function(v)
        CloneSpawnerEnabled = v
        if v then
            -- Create mobile spawn button if on mobile
            if _isMobile and not CloneSpawnerMobileBtn then
                CloneSpawnerMobileBtn = makeLagswitchMobileBtn(
                    "KWCloneSpawnerBtn",
                    "CLONE",
                    UDim2.new(0, 10, 1, -280),
                    spawnR6Model
                )
            end
            notify("Clone Spawner enabled. Press Q (or tap CLONE) to spawn.", 3)
        else
            if CloneSpawnerMobileBtn then
                CloneSpawnerMobileBtn:Destroy()
                CloneSpawnerMobileBtn = nil
            end
            notify("Clone Spawner disabled.", 2)
        end
    end
})

CloneSection:CreateButton({
    Name = "Spawn Clone (Q on PC)",
    Callback = function()
        if not CloneSpawnerEnabled then
            notify("Enable Clone Spawner first.", 2)
        else
            spawnR6Model()
        end
    end
...
-- ─── UI Scale ────────────────────────────────────────────────────────────────
local _uiScaleObj = nil

local function _getOrCreateUIScale()
    -- Try CoreGui first, then PlayerGui (no WaitForChild — already loaded)
    local sg = game:GetService("CoreGui"):FindFirstChild("KamiWareGUI")
    if not sg then
        sg = LocalPlayer:FindFirstChild("PlayerGui") and LocalPlayer.PlayerGui:FindFirstChild("KamiWareGUI")
    end
    if not sg then return nil end
    local mf = sg:FindFirstChild("MainFrame")
    if not mf then return nil end
    if _uiScaleObj and _uiScaleObj.Parent then return _uiScaleObj end
...
local function _getAutoScale()
    ...
    -- UI base size (matches windowSize default)
    local uiW = windowSize.X.Offset  -- 836
    local uiH = windowSize.Y.Offset  -- 538

    -- Leave a small margin (96%) so the UI doesn't touch screen edges
    local margin = 0.96
    local scaleByW = (vw * margin) / uiW
    local scaleByH = (vh * margin) / uiH

    -- Use whichever axis is more constrained, clamp to 30–100%
    local autoScale = math.min(scaleByW, scaleByH)
    autoScale = math.clamp(autoScale, 0.30, 1.00)

    -- Return as an integer percentage (matches slider units)
    return math.floor(autoScale * 100)
end

local function _applyUIScale(sliderVal)
    local scale = sliderVal / 100
    local sc = _getOrCreateUIScale()
    if sc then sc.Scale = scale end
end

-- Apply auto-scale after a short delay so the GUI is guaranteed to exist
task.delay(0.5, function()
    local autoVal = _getAutoScale()
    _applyUIScale(autoVal)
    -- Re-centre MainFrame so the scaled UI stays on screen
    local sg = game:GetService("CoreGui"):FindFirstChild("KamiWareGUI")
    if not sg then
        sg = game.Players.LocalPlayer.PlayerGui:FindFirstChild("KamiWareGUI")
    end
    if sg then
        local mf = sg:FindFirstChild("MainFrame")
        if mf then
            mf.Position = UDim2.new(0.5, -windowSize.X.Offset/2, 0.5, -windowSize.Y.Offset/2)
        end
    end
end)
...
SettingsLeft:CreateButton({
    Name = "Reset UI Scale (Auto)",
    Callback = function()
        local autoVal = _getAutoScale()
        _applyUIScale(autoVal)
        -- Re-centre MainFrame after scale reset
        local sg = game:GetService("CoreGui"):FindFirstChild("KamiWareGUI")
        if not sg then
            sg = game.Players.LocalPlayer.PlayerGui:FindFirstChild("KamiWareGUI")
        end
        ...
        notify("UI Scale reset to auto (" .. autoVal .. "%).", 2)
    end
})
-- ─────────────────────────────────────────────────────────────────────────────
