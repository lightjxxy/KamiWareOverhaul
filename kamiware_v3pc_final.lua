...
MovLeft:CreateToggle({
    ...
    Callback = function(v)
        ...
        if v then
            enableSpeedFix()
            patchSpeedFix()
            notify("Emote Speed Fix enabled.", 2)
        else
...
local speedFixHooked = false
local oldRequireSpeedFix

local function enableSpeedFix()
    if speedFixHooked then return end
    speedFixHooked = true
    -- Patch already-required modules in ReplicatedStorage
...
local SPRINT_ANIM_KEYWORDS = {"sprint", "run", "forward"}
...
local function isSprintOrJumpAnimPlaying()
    ...
    -- Also check humanoid state as a fallback for jump
