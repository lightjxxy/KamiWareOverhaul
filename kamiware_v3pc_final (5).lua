...
-- hookfunction is installed at script load time (before anything else caches modules)
-- so it catches all require() calls. The toggle just gates whether patching happens.
-- =====================================================
...
-- Install the hook immediately at load time before any modules are cached
local _speedFixOldRequire
pcall(function()
    _speedFixOldRequire = hookfunction(require, function(module)
        local result = _speedFixOldRequire(module)
        if SpeedFixEnabled and type(result) == "table" and typeof(module) == "Instance" and module:IsDescendantOf(ReplicatedStorage) then
            applySpeedFixToTable(result)
        end
        return result
    end)
end)
...
local function applySpeedFix()
    -- Patch any modules already required before the toggle was enabled
    local patched = 0
...
local function revertSpeedFix()
    for info, original in pairs(SpeedFixOriginals) do
        pcall(function() info.SpeedMult = original end)
    end
    SpeedFixOriginals = {}
end
